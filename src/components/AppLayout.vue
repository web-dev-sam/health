...
<template>
  <div class="min-h-screen bg-[#0d0d0d] text-[#e8e8e8] font-mono text-sm">
    ...
    <nav class="border-b border-white/10 px-4 py-4">
      <div class="max-w-4xl mx-auto flex items-center gap-3">

        <!-- Logo -->
        <RouterLink to="/" aria-label="Home" class="shrink-0">
        ...
        <!-- Mobile: scan + hamburger -->
        <div class="flex sm:hidden items-center gap-2">
          <button
            @click="showScan = true"
            class="text-white/40 hover:text-white/70 transition-colors border border-white/10 px-2 py-1 cursor-pointer"
            :aria-label="t('nav.scan')"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
              <path d="M3 6h18M3 12h12M3 18h8"/>
      ...
      <div v-if="showMenu" class="sm:hidden border-t border-white/10 mt-4 pt-4 px-4 space-y-4">
        <!-- Search -->
        <div class="relative">
          <input
            v-model="searchQuery"
            type="text"
            :placeholder="t('nav.searchPlaceholder')"
            class="w-full bg-transparent border border-white/10 text-white/70 text-xs px-3 py-2 rounded focus:outline-none focus:border-white/30 placeholder-white/20"
            @input="onInput"
            @focus="showDropdown = searchQuery.trim().length > 0"
            @blur="hideDropdown"
            @keydown.escape="clearSearch"
          ...
          <div
            v-if="showDropdown && searchResults.length > 0"
            class="absolute left-0 right-0 top-full mt-1 bg-[#1a1a1a] border border-white/10 rounded shadow-lg z-50 overflow-hidden"
          >
            <button
              v-for="result in searchResults"
              :key="result.route + result.name"
        ...
        <div class="flex flex-col gap-3">
          <RouterLink to="/sweeteners" class="text-white/40 text-xs" active-class="text-white" @click="showMenu = false">{{ t('nav.sweeteners') }}</RouterLink>
          <RouterLink to="/preservatives" class="text-white/40 text-xs" active-class="text-white" @click="showMenu = false">{{ t('nav.preservatives') }}</RouterLink>
          <RouterLink to="/emulsifiers" class="text-white/40 text-xs" active-class="text-white" @click="showMenu = false">{{ t('nav.emulsifiers') }}</RouterLink>
        </div>
        <!-- Actions -->
        <div class="flex items-center gap-2 pb-1">
          <button
            ...
            class="text-xs text-white/40 hover:text-white/70 transition-colors border border-white/10 px-3 py-1 rounded cursor-pointer"
          >{{ current === 'en' ? 'DE' : 'EN' }}</button>
          <button @click="share; showMenu = false" class="text-xs text-white/40 hover:text-white/70 transition-colors border border-white/10 px-3 py-1 rounded cursor-pointer">
            {{ copied ? t('nav.copied') : t('nav.share') }}
          </button>
          <a
            href="https://github.com/web-dev-sam/health"
